...
se.sum = sum(predict(model, newdata = joined_data, se.fit = TRUE)$se.fit * joined_data$baseline )/1000000
fit.sum =  sum(predict(model, newdata = joined_data, se.fit = TRUE)$fit *  joined_data$baseline )/1000000
...
se2.sum = sum(fitted2$se2)/1000000
fit2.sum = sum(fitted2$combined_projection)/1000000


output = tibble(`Estimate (millions)` = fit.sum, 
                `CI Lower` = fit.sum-2*se.sum,
                `CI Upper` = fit.sum+2*se.sum,
                `Estimate 2 (millions)` = fit2.sum,
                `CI Lower 2` = fit2.sum-2*se2.sum,
                `CI Upper 2` = fit2.sum+2*se2.sum)

output  %>% kable(digits = 2, format.args = list(big.mark = ",", scientific = FALSE),
                  col.names = c("Estimate (millions)",
                           "CI Lower",
                           "CI Upper",
                           "Estimate (millions)",
                           "CI Lower",
